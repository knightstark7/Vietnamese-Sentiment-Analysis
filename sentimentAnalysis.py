...
def draw_bar_chart(percentages):
    ...
    categories = ['Tiêu cực', 'Trung lập', 'Tích cực']
    
    # Convert percentages to values in the range [0, 100]
    values = [percentage * 100 for percentage in percentages]
...
def getSentiments(userText):
    ...
    radar_chart = draw_bar_chart(list(percentages[0]))
...
def renderPage():
    st.title("Vietnamese Customer Reviews Sentiment Analysis 😊😐😕")
    ...
    decor = Image.open("images/forapp/customer_review_analysis.jpg")
    st.image(decor)
    
    st.subheader("Phân tích đánh giá ý kiến người dùng")
    st.text("Phân tích đánh giá của người dùng và trả về cảm xúc của nó")
    st.text("")
    userText = st.text_input('User Input', placeholder='Input text HERE')
    
    if st.button('Tiến hành phân tích cảm xúc'):
        if userText != "" and type is not None:
            st.components.v1.html("""
